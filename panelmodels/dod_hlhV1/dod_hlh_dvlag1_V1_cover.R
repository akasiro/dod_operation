...
Model1.1 <- lmer(coverage_1 * 100 ~ span + price + payInApp + size + compatibility + contentRank + age + samepubappnum + (1|lv2id_x), data=cover_lag1)
# 4.1.2 Model1.2 baseline model
Model1.2 <- lmer(coverage_1 * 100 ~ Cdiff + Cdiff2 + span + price + payInApp + size + compatibility + contentRank + age + samepubappnum + (diff | lv2id_x) + (diff2 | lv2id_x) + (1|lv2id_x), data=cover_lag1)
# 4.1.3 Model1.3 Moderation gvaction
Model1.3 <- lmer(coverage_1 * 100 ~ Cdiff + Cdiff2 +gv_action + Cdiff*gv_action + Cdiff2*gv_action + span + price + payInApp + size + compatibility + contentRank + age + samepubappnum + (diff | lv2id_x) + (diff2 | lv2id_x) + (1|lv2id_x), data=cover_lag1)
# 4.1.4 Model1.4 Moderation c_hetro
Model1.4 <- lmer(coverage_1 * 100 ~ Cdiff + Cdiff2 + c_hetro + Cdiff*c_hetro + Cdiff2*c_hetro + span + price + payInApp + size + compatibility + contentRank + age + samepubappnum + (diff | lv2id_x) + (diff2 | lv2id_x) + (1|lv2id_x), data=cover_lag1)
